...
export class CreateMutualEventPage {
  pic
  menuPopUp: boolean
  location
  lat=45.480827
  ...
  onLocation() {
    ...
    Modal.onDidDismiss(data => {
      if (data) {
        ...
        var a = data;
        this.MutualEvent.address = a.address;
        this.lat = a.lat;
        this.lng = a.lng;
      }
    });
  ...
  fromgallery() {
    this.camera.getPicture({
      ...
      this.MutualEvent.image = imageData;
  ...
  onSave() {
    ...
    Observable.fromPromise(loading.present())
      .flatMap(data => this.securityProvider.CreateEvent(this.MutualEvent))
      .subscribe(data =>
        loading.dismiss().then(() => {
         ...
         /*    let alert = this.alertCtrl.create({
		  ...
		  toast1.present();
			...
			this.navCtrl.push('EditEventPage',{eventData:a.eventData});
          } else {
           /*  let alert = this.alertCtrl.create({
