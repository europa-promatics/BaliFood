...
export class EditEventPage {
  ...
  eventType
  popupMessages
   constructor(public alertCtrl: AlertController, public loadingCtrl: LoadingController, public modalCtrl: ModalController, public securityProvider: SecurityProvider, public actionSheetCtrl: ActionSheetController, private camera: Camera, public navCtrl: NavController, public navParams: NavParams, platform: Platform,     private translate: TranslateService,public toastCtrl: ToastController) {
  ...
  value => {
    ...
    this.Event = {}
    if (this.eventData) {
	localStorage['evento']=this.eventData;
	localStorage['typo']=this.eventType;
 ...
 public presentActionSheet() {
  ...
  fromcamera() {
    ...
    title: this.popupMessages.confirm_edit_title,
    message: this.popupMessages.sure_edit,
    buttons: [
      {
    ...
    if (this.Event) {
      ...
      Observable.fromPromise(loading.present())
        ...
        .subscribe(data =>
          ...
          alert.present(); */ 
		  /* "date":"2018-08-28T00:00:00.000Z","time":"12:51"
		  var date=this.Event.date.split('T')[0]+this.Event.time
		  ...
		  let toast1 = this.toastCtrl.create({
				message: this.popupMessages.Item_updated_successfully,
				duration: 3000,
				position: 'top'
		  })
		  toast1.onDidDismiss(() => {
